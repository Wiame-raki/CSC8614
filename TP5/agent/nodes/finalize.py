...
def finalize(state: AgentState) -> AgentState:
    ...
    if intent == "reply":
        ...
        if cits:
            ...
            state.final_text = state.draft_v1.strip() or "Merci pour votre email. J'ai reçu votre demande."  # TODO: fallback reply

    elif intent == "ask_clarification":
        state.final_kind = "clarification"
        state.final_text = state.draft_v1.strip() or "Pouvez-vous préciser votre demande ?\n- Quel est le sujet exact ?\n- Pouvez-vous fournir plus de détails ?"
  # TODO: fallback questions
        ...
        state.actions.append({
            ...
            "summary": f"Escalade: {state.subject[:100]}",
        ...
        state.final_text = "Votre demande nécessite une validation humaine. Je transmets avec un résumé et les sources."  # TODO: texte minimal ignore
