...
def stub_reply(state: AgentState) -> AgentState:
    ...
    state.draft_v1 = "[STUB] Réponse automatique générée. [À remplacer par génération avec RAG]"
...
def stub_ask_clarification(state: AgentState) -> AgentState:
    ...
    state.draft_v1 = "[STUB] Veuillez préciser votre demande. Pouvez-vous fournir plus de détails?"
...
def stub_escalate(state: AgentState) -> AgentState:
    ...
    state.actions.append({
        "type": "handoff_human",
        "summary": f"Escalade requise - {state.decision.category.upper()} - Priorité {state.decision.priority}/5",
    })
...
def stub_ignore(state: AgentState) -> AgentState:
    ...
    state.actions.append({
        "type": "ignore",
        "reason": "Email hors périmètre ou spam - aucune action requise",
    })
