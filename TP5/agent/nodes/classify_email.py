...
def classify_email(state: AgentState) -> AgentState:
    ...
    try:
        ...
        log_event(state.run_id, "error", {"node": "classify_email", "kind": "parse_or_validation", "msg": str(e), "raw": raw[:200]})
        repair = REPAIR_PROMPT.format(raw=raw)
        raw2 = call_llm(repair)
        try:
            decision = parse_and_validate(raw2)
        except Exception as e2:
            log_event(state.run_id, "error", {"node": "classify_email", "kind": "parse_or_validation_RETRY", "msg": str(e2), "raw": raw2[:200]})
            decision = Decision(
                intent="escalate",
                category="other",
                priority=1,
                ...
                needs_retrieval=False,
                retrieval_query="",
                rationale="Impossible to parse LLM output, even after retry."
            )
